...
#
# The value is fed to base64 with `printf '%s'` rather than `echo -n`: zsh's
# builtin `echo` interprets backslash escapes by default, so a value holding a
# backslash was corrupted before it reached base64 (`a\\b` -> `a\b`, `\t` -> a
