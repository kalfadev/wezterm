...
impl SessionSender {
    ...
    /// Queue an event that must not be lost to a full queue,
...
pub struct Session {
    tx: SessionSender,
    /// Announces the drop when the LAST handle goes, and not
    /// before.
    ///
    /// This type is a handle and is cloned as a matter of course — the domain
    /// keeps one, the thread carrying out authentication is given one, and a
    /// caller may hold one of its own. A `Drop` on the
    ...
    /// How long the far end has been holding something of ours it
    ...
    /// Whether the session thread has got as far as serving
    /// requests. See [`Session::is_established`].
    established: Arc<AtomicBool>,
    /// Whether authentication was won by a public key.
    /// See [`Session::authenticated_with_key`].
    authenticated_with_key: Arc<AtomicBool>,
}

/// The one owner of the "these handles are gone" signal.
///
...
impl Session {
    ...
    /// Whether there is still a session thread behind this handle.
    ///
    ...
    /// Has this session finished being established?
    ///
    ...
    /// How long the far end has been holding something this
    ...
    /// Did a public key win this session's authentication?
    ///
