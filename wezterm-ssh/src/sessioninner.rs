...
/// Shortest wait in the session poll loop, and the value the
...
/// How long what we have sent may go unacknowledged before the
...
/// The socket options every SSH transport is opened with.
///
...
/// Drop the connection once data has gone unacknowledged for
...
/// See the Linux arm above. Nothing to do on this platform.
#[cfg(not(any(
    target_os = "linux",
    target_os = "android",
    target_os = "macos",
...
/// How many bytes the far end has not taken delivery of.
///
...
/// See the Apple arm above. This platform is not asked.
#[cfg(not(any(
    target_os = "linux",
    target_os = "android",
    target_os = "macos",
...
/// Set one integer `IPPROTO_TCP` option on `sock`.
///
...
pub(crate) struct SessionInner {
    ...
    /// The owner has closed this connection. Unlike
    ...
    /// Set once this session reaches its request loop, which is
    ...
    /// The same thing, in milliseconds, for a holder of the
...
impl SessionInner {
    ...
    fn run_impl_libssh(&mut self) -> anyhow::Result<()> {
        ...
        // No blocking call waits for ever.
        //
    ...
    /// abandoned.
    ///
    /// `Socket::connect` carries no timeout of its own, so
    ...
    /// this code on the FIRST connection to a TLS domain. Leaving it unbounded
    /// meant the same address failed after ~75s on the first attempt and after
    /// 10s on the second, once credentials were cached.
    ///
    ...
    /// Read whether the far end is keeping up, and for how long it
    /// has not been.
    ///
    ...
    fn request_loop(&mut self, sess: &mut SessionWrap) -> anyhow::Result<()> {
        // Authenticated, and from here on serving requests. This
        ...
        loop {
            ...
            // `tick_io` reports whether it actually moved bytes.
            ...
            // Asked to close, so close — the channels are not
            ...
            // The transport has gone, so say so.
            //
            ...
            // Also watch the session socket for readability
            ...
            // Bounded backoff. `sleep_delay` used to double
    ...
    ///
    /// Returns `true` when this tick actually moved bytes in
    /// either direction.
    ///
    ...
    /// Does any channel still have room for more remote output?
    ///
        ...
        match self.rx_req.try_recv() {
            ...
            Ok(req) => {
                ...
                let res = match req {
                    ...
                    // Stop draining as well as stop looping —
    ...
    T: Send + Sync + 'static,
{
    if let Err(err) = reply.try_send(f()) {
        // At `debug`, because this says nothing about the
        // operation.
        //
