...
///
/// ssh_config encodes a *list* of paths in a single option value
    ...
    fn parse_impl(
        ...
        for line in s.lines() {
            ...
            if let Some(sep) = line.find(|c: char| c == '=' || c.is_whitespace()) {
                ...
                // Remember whether the quotes were there. A path
                ...
                fn add_option(options: &mut ConfigMap, k: String, v: &str, was_quoted: bool) {
                    ...
                    // Put back the quotes the parser stripped, so
...
impl Config {
    ...
    pub fn for_host<H: AsRef<str>>(&self, host: H) -> ConfigMap {
        ...
        // The defaults below are built from `$HOME`, which can
    ...
    /// What the configuration files STATE for a given host, without
    ...
    pub fn for_host_as_stated<H: AsRef<str>>(&self, host: H) -> ConfigMap {
        ...
        for (k, v) in &mut result {
            // A path list is expanded ENTRY BY ENTRY and re-encoded.
            //
                ...
                *value = value.replace(t, &self.resolve_local_host(true));
                ...
                if let Some(home) = self.resolve_home() {
                    let mut items = value
                        ...
                        .map(|s| s.to_string())
                        .collect::<Vec<String>>();
                    // Path lists reach this function one
...
mod test {
    ...
    // Path lists with spaces. The round-trip is what matters —
